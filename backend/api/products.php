...
function buildProduct($row, $conn) {
    $productId = $row['id'];
    $images = [];
    ...
    $discounted_price = $discount > 0 ? round($price * (1 - $discount / 100), 2) : null;
...
if ($q !== '') {
    ...
    while ($row = $result->fetch_assoc()) {
        $products[] = buildProduct($row, $conn);
    }
    $stmt->close();
    ...
    $total_items = (int)($result_total->fetch_assoc()['total'] ?? 0);
}
